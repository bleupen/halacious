...
// see http://tools.ietf.org/html/draft-kelly-json-hal-06#section-8.2
     ...
     * @param relativeTo
     */
    internals.link = function (link, relativeTo, request) {
        relativeTo = relativeTo && relativeTo.split('?')[0];
        let base = '';
        if (_.get(request, ['headers', 'x-forwarded-path'], false)) base = request.headers['x-forwarded-path'];
        link = _.isFunction(link) ? { href: link }  : _.isString(link) ? { href: base+link } : hoek.clone(link);
...
// keeps found routes in a cache
     ...
     */
    internals.route = function (routeId, params, request) {
        let base = '';
        if (_.get(request, ['headers', 'x-forwarded-path'], false)) base = '/' + request.headers['x-forwarded-path'];
        ...
        var href = base + (_.template(route.path, { interpolate: /{([\s\S]+?)(?:\?|\*\d*)??}/g })(_.mapValues(params, function(val) {
