 ...
 * @return {*}
 */
internals.location = function (uri) {
...
internals.preResponse = function (halacious, settings, request, reply) {
    ...
    if (mediaType &&
        ...
        self = location ? internals.location(location) : internals.getRequestPath(request, halConfig.query);
        ...
        internals.configureRepresentation(rep, halConfig, function (err, rep) {
            ...
            var response = reply(rep).type(mediaType).code(request.response.statusCode).location(request.response.settings.location);
            response.hold();
