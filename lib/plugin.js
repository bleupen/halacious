 ...
 * @return {*}
 */
internals.location = function (uri, server) {
    var isAbsolute = (uri.match(/^\w+\:\/\//));
    if (!server.settings.location || isAbsolute) {
        return uri;
    }

    return server.settings.location + (uri.charAt(0) === '/' ? '' : '/') + uri;
};
...
internals.preResponse = function (halacious, settings, request, next) {
    ...
    if (mediaType &&
        ...
        location = request.response.settings.location;
        self = location ? internals.location(location, request.server) : internals.getRequestPath(request, halConfig.query);
        ...
        internals.configureRepresentation(rep, halConfig, function (err, rep) {
            ...
            var response = next(rep).type(mediaType).code(request.response.statusCode).location(request.response.settings.location);
