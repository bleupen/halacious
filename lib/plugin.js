...
internals.link = function (link, relativeTo) {
    ...
    if (relativeTo && internals.isRelativePath(link.href)) {
        link.href = new URI(link.href).absoluteTo(relativeTo+'/').toString();
    }
 ...
 * @return {*}
 */
internals.locateRoute = function (server, routeName) {
    var route, routes, i;
    ...
    routes = server.table().reduce(function (acc, conn) {
 ...
 */
internals.route = function (server, routeId, params) {
    var route = server.lookup(routeId) || internals.locateRoute(server, routeId);
    if (!route) throw new Error('No route found with id or name ' + routeId);
    var href = _.template(route.path)(params);
...
internals.namespaceUrl = function (relsBase, namespace) {
    return relsBase + '/' + namespace.name;
};
...
internals.configureRepresentation = function configureRepresentation(rep, config, callback) {
    var resolveHref = function(href, ctx) {
        return _.isFunction(href) ? href(rep, ctx) : urlTemplate.parse(href).expand(flattenContext(href, ctx));
    };
...
internals.expandPath = function(request, path, search) {
    return  url.format({
        ...
        hostname: request.connection.info.host,
        port: request.connection.info.port,
        pathname: path,
...
internals.getRequestPath = function(request, queryTemplate, absolute) {
    ...
    var path = absolute ? internals.expandPath(request, request.path) : request.path;
...
internals.preResponse = function (halacious, settings, request, reply) {
    ...
    if (mediaType &&
        ...
        internals.configureRepresentation(rep, halConfig, function (err, rep) {
            ...
            var response = reply(rep).type(mediaType).code(request.response.statusCode);
...
internals.apiLinker = function (absolute, rep, next) {
    ...
    var routes = req.server.table().reduce(function(acc, conn) {
        return acc.concat(conn.table);
    }, []);
    ...
    for (var i = 0; i < routes.length; i++) {
        ...
        if (halConfig.api) {
            ...
            if (absolute) {
                href = internals.expandPath(rep.request, href);
            }
...
exports.register = function (server, opts, next) {
    ...
    var api = {
        ...
        route: internals.route.bind(internals, server),
        apiLinker: internals.apiLinker
    };

    // hapi wont find the local swig without this
    server.expose(api);
    server.route({ method: 'get', path: settings.relsPath, config: internals.namespacesRoute(settings.relsAuth) });
    server.route({ method: 'get', path: settings.relsPath + '/{namespace}/{rel}', config: internals.relRoute(settings.relsAuth) });
    ...
    if (settings.autoApi) {
        ...
        selection.route({ method: 'GET', path: settings.apiPath + '/', config: internals.apiRouteConfig(settings.absolute, settings.apiAuth) });
