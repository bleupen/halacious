/* eslint-disable no-shadow,one-var */
let joi = require('joi');
...
let { RepresentationFactory } = require('./representation');
...
let HAL_MIME_TYPE = 'application/hal+json';
...
exports.plugin = {
  ...
  async register(server, opts) {
    ...
    let selection = settings.apiServerLabel
    ...
    internals.rels.add = function(namespace, rel) {
      let ns = internals.byName[namespace];
    ...
    internals.route = function(routeId, params) {
      let route = server.lookup(routeId) || internals.locateRoute(routeId);
      if (!route) throw new Error(`No route found with id or name ${routeId}`);
      let href = _.template(route.path, {
        interpolate: /{([\s\S]+?)(?:\?|\*\d*)??}/g,
      })(
        _.mapValues(
          params,
          val => (typeof val !== 'object' ? encodeURIComponent(val) : val)
        )
      );
      let query = hoek.reach(route.settings, 'plugins.hal.query');
    ...
    internals.namespaceUrl = function(request, namespace) {
      let path = [settings.relsPath, namespace.name].join('/');
    ...
    internals.configureRepresentation = function configureRepresentation(
      ...
      let resolveHref = function(href, ctx) {
      ...
      try {
         ...
         * @param callback
         */
        let prepareEntity = function(rep, callback) {
          ...
          (rel, cb) => {
            ...
            if (!embed.path) {
              throw new Error(
                `Error in route ${
                  rep.request.path
                }: "embedded" route configuration property requires a path`
              );
            }
    ...
    internals.halifyAndConfigure = function(rep, config, callback) {
       ...
       * @param callback
       */
      let convertEntity = function(callback) {
      ...
      let configureEntity = function(rep, callback) {
    ...
    internals.apiLinker = function(absolute, rep, next) {
      ...
      let routes = req.server.table();

      for (let i = 0; i < routes.length; i++) {
        let route = routes[i];

        let halConfig = route.settings.plugins.hal || {};

        if (halConfig.api) {
          let rel = halConfig.api;
    ...
    internals.postHandler = function(request, h) {
      ...
      let mediaType = internals.getMediaType(settings.mediaTypes, request);
      let absolute;
    ...
    internals.preStart = function(server) {
      if (_.isFunction(server.views)) {
        server.log(
          ['halacious', 'info'],
          'Views support detected, installing documentation routes'
        );
        ...
        server.log(
          ['halacious', 'info'],
          'Views support not detected. Please install vision plugin for rel documentation'
        );
      }
    };
