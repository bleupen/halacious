...
internals.preResponse = function(halacious, request, next) {
    ...
    if (request.headers.accept &&
        ...
        if (_.isFunction(halConfig)) halConfig = { prepare: halConfig };

        var entity = request.response.source;

        var rep = rf.create(entity);

        var configureRep = function(rep, config, callback) {
            _.forEach(config.links, function (link, rel) {
                rep.link(rel, link);
            });

            _.forEach(config.embed, function (embed, rel) {
                // works for single value or array
                var embedded = hoek.reach(entity, embed.path);
                embedded = _.isArray(embedded) ? embedded : [ embedded ];
                embedded.forEach(function (item) {
                    var embeddedRep = rep.embed(rel, embed.href, item);
                    configureRep(embeddedRep, embed, function (err, rep) {
        ...
        configureRep(rep, halConfig, function(err, rep) {
            if (err) {
                return next(err);
            }
