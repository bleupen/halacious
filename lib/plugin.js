...
internals.apiHandler = function(apiAuth) {
    return {
        auth: apiAuth,
        handler: function (req, reply) {
...
internals.apiRedirectHandler = function (apiUrl, apiAuth) {
    return {
        auth: apiAuth,
        handler: function (req, reply) {
...
exports.register = function (plugin, opts, next) {
    ...
    if (settings.autoApi) {
        ...
        selection.route({ method: 'GET', path: settings.apiUrl + '/', config: internals.apiHandler(settings.apiAuth) });

        // set up a redirect to api root + '/'
        if (settings.apiUrl.length > 0) {
            selection.route({ method: 'GET', path: settings.apiUrl, config: internals.apiRedirectHandler(settings.apiUrl, settings.apiAuth)});
        }
    }
