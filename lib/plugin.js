...
internals.configureRepresentation = function configureRepresentation(rep, config, callback) {
    try {
        ...
        _.forEach(config.embedded, function (embed, rel) {
            ...
            embedded.forEach(function (item) {
                ...
                configureRepresentation(embeddedRep, embed, function (err) {
...
internals.preResponse = function (halacious, request, next) {
    ...
    if (request.headers.accept &&
        ...
        // self will default to request.path
        rep = rf.create(entity);
...
exports.register = function (plugin, opts, next) {
    ...
    var api = {
        ...
        namespaceUrl: internals.namespaceUrl.bind(undefined, settings.relsUrl),
        link: internals.link,
        rels: internals.rels,
    ...
    plugin.expose(api);
    plugin.route({ method: 'get', path: settings.relsUrl, config: internals.namespacesRoute(settings.relsAuth) });
    plugin.route({ method: 'get', path: path.join(settings.relsUrl, '{namespace}/{rel}'), config: internals.relRoute(settings.relsAuth) });
    ...
    if (settings.autoApi) {
        ...
        selection.route({ method: 'GET', path: settings.apiUrl + '/', config: internals.apiRouteConfig(settings.apiAuth) });

        // set up a redirect to api root + '/'
        if (settings.apiUrl.length > 0) {
            selection.route({ method: 'GET', path: settings.apiUrl, config: internals.apiRedirectConfig(settings.apiUrl, settings.apiAuth)});
        }
    }
