...
internals.preResponse = function(halacious, request, next) {
    ...
    if (request.headers.accept &&
        ...
        _.forEach(halConfig.links, function (link, rel) {
            rep.link(rel, link);
        });

        _.forEach(halConfig.embed, function (embed, rel) {
            // works for single value or array
            rep.embedCollection(rel, embed.href, hoek.reach(entity, embed.path));
            rep.ignore(embed.path);
        });

        rep.ignore(halConfig.ignore);
        ...
        var prepareEntity = function (rep, callback) {
            if (_.isFunction(halConfig.prepare)) {
                halConfig.prepare(rep, wrap(callback));
            } else {
                callback(null, rep);
            }
        };
