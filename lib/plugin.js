...
internals.preResponse = function (halacious, request, next) {
    ...
    if (request.headers.accept &&
        ...
        // location should be set in a response header but for some reason its in settings
        // at this point
        var self = statusCode === 201 && request.response.settings.location || request.path;
        ...
        internals.configureRepresentation(rep, halConfig, function (err, rep) {
            ...
            next(rep).type('application/hal+json').code(request.response.statusCode).location(request.response.settings.location);
        });
    } else {
        next();
    }
