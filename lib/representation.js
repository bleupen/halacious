...
Representation.prototype.link = function(rel, link) {
    const isRelativePath = function (path) {
        return path && (path.substring(0,1) === '/' || path.substring(0, 2) === './' || path.substring(0, 3) === '../');
    };
    var qname;
    var originalRel = rel;
    ...
    link = isRelativePath(link.href || link) ? this._halacious.link(link, this._links.self.href) : this._halacious.link(link);
 ...
 * @param callback
 */
Representation.prototype.configure = function(config, callback) {
